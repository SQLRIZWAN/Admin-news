    ...
    <div className="fade-up">
      ...
      {(()=>{
        ...
        const publishAll = async ()=>{
          ...
          try{
            ...
            if(!docs.length){ toast.show('No articles found to publish','error'); setFixing(false); return; }
            const SERVER_TS = firebase.firestore.FieldValue.serverTimestamp();
            let fixed=0, tsAdded=0;
            for(let i=0;i<docs.length;i+=400){
              const batch = db.batch();
              docs.slice(i,i+400).forEach(d=>{
                const x = d.data();
                const patch = { hidden:false, published:true, status:'published' };
                if(!x.timestamp){ patch.timestamp = x.createdAt || SERVER_TS; tsAdded++; }
                if(!x.createdAt){ patch.createdAt = x.timestamp || SERVER_TS; }
      ...
      {(()=>{
        ...
        const testDb = async ()=>{
          ...
          let serverDocs = null;
          let serverDetail = '';
          const scanFull = async ()=> (await db.collection('news').limit(1000).get({source:'server'})).docs;
          try{
            ...
            try{
              ...
              try{
                ...
                const all = [];
                for(const c of cats){
                  const s = await db.collection('news').where('category','==',c).limit(500).get({source:'server'});
          ...
          if(serverOk && serverDocs){
            ...
            results.push({name:`🔢 Total on SERVER (live Firestore)`, ok:true, count:serverDocs.length,
